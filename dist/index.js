...
async function createRegistryProviderVersion(tfToken, tag, gpgKey, tfUrl, provider) {
    ...
    try {
        ...
        if (error instanceof Error) {
            console.error(`Error creating registry provider version: ${error.message}`);
        }
        else {
...
;// CONCATENATED MODULE: ./src/assetDownload.ts


async function getLatestRelease(githubRepo, authToken) {
...
async function getAssetDownloadUrls(latestRelease, tag, githubRepo, authToken, osArchPairs) {
    ...
    console.log('Asset IDs:', assets.map(asset => asset.name));
    return { repoName, assets };
}
async function downloadAssets(githubRepo, assets, outputDir, authToken) {
    for (const asset of assets) {
        console.log(`Downloading asset: ${asset.browser_download_url}`);
        await downloadAsset(githubRepo, asset, outputDir, authToken);
    }
}
async function downloadAsset(githubRepo, asset, outputDir, authToken) {
...
;// CONCATENATED MODULE: ./src/uploadBinaryFiles.ts
...
async function uploadBinaryFiles(tfToken, tfUrl, provider, tag, osArch, shasum, filename) {
...
async function run() {
    try {
        ...
        for (const osArch of osArchPairs) {
            ...
            const providerBinaryUploadLink = await uploadBinaryFiles(tfToken, tfUrl, provider, tag, osArch, shasum, fileName);
